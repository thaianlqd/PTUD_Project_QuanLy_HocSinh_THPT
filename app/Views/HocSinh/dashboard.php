...
<div class="sidebar" id="sidebar">
  <div class="profile-section">
    <img src="https://via.placeholder.com/80x80?text=HS" class="rounded-circle mb-2" style="width:80px;height:80px;border:3px solid var(--primary);">
    <h5 class="fw-bold text-primary mb-1">
      <?php echo htmlspecialchars($data['student_info']['ho_ten'] ?? $data['user_name'] ?? 'Học Sinh'); ?>
    </h5>
    <p class="text-muted small mb-0">Mã HS: <?php echo htmlspecialchars($data['student_info']['ma_hoc_sinh'] ?? 'HS???'); ?></p>
    <p class="text-muted small">Lớp: <?php echo htmlspecialchars($data['student_info']['ten_lop'] ?? 'N/A'); ?></p>

    <div class="row g-1 mt-2 text-center">
      <div class="col-4">
        <small class="text-muted">Bài Chưa Nộp</small><br>
        <strong class="text-primary fs-6"><?php echo $data['bai_chua_nop'] ?? '?'; ?></strong>
      </div>
      <div class="col-4">
        <small class="text-muted">Điểm TB</small><br>
        <strong class="text-success fs-6"><?php echo $data['diem_tb_hk'] ?? '?'; ?></strong>
      </div>
      <div class="col-4">
        <small class="text-muted">Lịch Tuần</small><br>
        <strong class="text-info fs-6"><?php echo $data['lich_tuan_count'] ?? '?'; ?> buổi</strong>
      </div>
    </div>
  </div>
  ...
  <ul class="nav flex-column px-2 py-3">
    ...
    <li class="nav-item"><a class="nav-link" href="<?php echo BASE_URL; ?>/hocsinh/diemdanh"><i class="bi bi-person-check-fill me-2"></i>Điểm Danh</a></li>
...
<nav class="navbar navbar-expand-lg bg-white shadow-sm mb-4 rounded-3 p-2">
  <div class="container-fluid">
    ...
    <form class="d-flex mx-auto w-50">
      <input class="form-control rounded-pill me-2 shadow-sm" type="search" placeholder="Tìm bài/tên môn...">
      <button class="btn btn-outline-primary rounded-pill" type="submit"><i class="bi bi-search"></i></button>
    </form>

    <ul class="navbar-nav ms-auto align-items-center">
      <li class="nav-item me-3 position-relative">
        <a class="nav-link p-2" href="#"><i class="bi bi-bell fs-5"></i>
      ...
      <li class="nav-item dropdown">
        <a class="dropdown-toggle nav-link p-2" href="#" data-bs-toggle="dropdown">
          <img src="https://via.placeholder.com/32x32" class="rounded-circle me-2"><?php echo htmlspecialchars($data['user_name'] ?? 'Học Sinh'); ?>
        </a>
        <ul class="dropdown-menu dropdown-menu-end shadow">
          <li><a class="dropdown-item"><i class="bi bi-person me-2"></i>Cài đặt</a></li>
          <li><a class="dropdown-item"><i class="bi bi-gear me-2"></i>Đổi mật khẩu</a></li>
    ...
    <div class="card-body table-responsive">
      <table class="table table-striped">
        <thead class="table-primary">
          <tr>
            <th>Thứ</th><th>Tiết</th><th>Môn</th><th>GV</th><th>Phòng</th><th>Hành Động</th>
          </tr>
        </thead>
        <tbody>
        <?php $lich_hoc_tuan = $data['lich_hoc_tuan'] ?? []; ?>
        <?php if (empty($lich_hoc_tuan)): ?>
          <tr><td colspan="6" class="text-center py-4 text-muted">Chưa có lịch học.</td></tr>
        <?php else: ?>
          <?php foreach ($lich_hoc_tuan as $lich): ?>
          <tr>
            <td><?= htmlspecialchars($lich['thu']) ?></td>
            <td><?= htmlspecialchars($lich['tiet']) ?></td>
            <td><?= htmlspecialchars($lich['mon']) ?></td>
            <td><?= htmlspecialchars($lich['gv']) ?></td>
...
<section class="mb-5">
  ...
  <div class="row g-3">
    ...
    <div class="col-md-3">
      <div class="card h-100 text-center">
        <div class="card-body">
          <i class="bi bi-person-check-fill fs-2 text-success mb-3"></i>
          <h5 class="fw-bold">Điểm Danh</h5>
          <p class="text-muted small">Điểm danh trực tuyến</p>
          <a href="<?php echo BASE_URL; ?>/hocsinh/diemdanh" class="btn btn-outline-success mt-2">Vào điểm danh</a>
        </div>
      </div>
    </div>
...
<footer class="text-center mt-5 text-muted small pb-3">
  © <?= date("Y") ?> THPT Manager | Dành cho Học Sinh
</footer>
...
// Charts
const baiTapStats = <?php echo json_encode($data['bai_tap_stats'] ?? ['da_nop' => 0, 'chua_nop' => 0]); ?>;
const diemMon = <?php echo json_encode($data['diem_tb_mon'] ?? []); ?>;

// PIE
const pieCtx = document.getElementById('submitPie');
if (pieCtx && (baiTapStats.da_nop > 0 || baiTapStats.chua_nop > 0)) {
  new Chart(pieCtx, {
    type: 'pie',
    data: {
      labels: ['Đã hoàn thành', 'Chưa hoàn thành'],
      datasets: [{
        data: [baiTapStats.da_nop, baiTapStats.chua_nop],
